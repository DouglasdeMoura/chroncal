...
// TestPrintTable_SingleEventRendersVertically locks in that a single
// jsonEvent renders as a two-column FIELD/VALUE table rather than the
// 30-column horizontal row used for lists. A horizontal single-row
// table doesn't fit any normal terminal width.
func TestPrintTable_SingleEventRendersVertically(t *testing.T) {
	t.Parallel()
	withLocalUTC(t)
	...
	if len(lines) < 5 {
		t.Fatalf("expected vertical table with one row per field; got %d lines:\n%s", len(lines), buf.String())
	}
	if !strings.HasPrefix(lines[0], "FIELD") {
		t.Fatalf("first line %q does not start with FIELD header", lines[0])
	}
	// One row per field means no single line should contain both the title
	// and the UID; a horizontal table would.
	for _, line := range lines {
		if strings.Contains(line, "Standup") && strings.Contains(line, "uid-42") {
			t.Fatalf("line %q contains both title and uid; expected vertical layout", line)
		}
	}
	// And the expected rows should be present.
	for _, needle := range []string{"TITLE", "Standup", "UID", "uid-42"} {
