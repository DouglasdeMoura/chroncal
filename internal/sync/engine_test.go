...
// persist and the token must advance. A miss for a local row still
// withholds the token (see TestEnginePullToleratesMultigetMissingPath).
...
<d:multistatus xmlns:d="DAV:" xmlns:cal="urn:ietf:params:xml:ns:caldav">
  ...
  <d:response>
    <d:href>/calendar/phantom-invite.ics</d:href>
    <d:propstat>
      <d:prop>
        <d:getetag>&quot;etag-phantom&quot;</d:getetag>
      </d:prop>
      <d:status>HTTP/1.1 200 OK</d:status>
    </d:propstat>
  </d:response>
  <d:sync-token>https://example.com/sync/after-phantoms</d:sync-token>
</d:multistatus>`
...
<d:multistatus xmlns:d="DAV:" xmlns:cal="urn:ietf:params:xml:ns:caldav">
  ...
  <d:response>
	...
	if tok := storage.NullableToString(calRow.SyncToken); tok != "https://example.com/sync/after-phantoms" {
...
<d:multistatus xmlns:d="DAV:">
  <d:response>
    <d:href>/calendar/phantom-invite.ics</d:href>
    <d:propstat>
      <d:prop><d:getetag>&quot;etag-phantom&quot;</d:getetag></d:prop>
      <d:status>HTTP/1.1 200 OK</d:status>
    </d:propstat>
  </d:response>
...
<d:multistatus xmlns:d="DAV:">
  <d:response><d:href>/calendar/phantom-invite.ics</d:href><d:status>HTTP/1.1 404 Not Found</d:status></d:response>
</d:multistatus>`)), Request: r}, nil
	})
	...
	if tok := storage.NullableToString(calRow.SyncToken); tok != "https://example.com/sync/t1" {
...
func TestEnginePullRetriesUnknownMultigetMissAfterTokenAdvance(t *testing.T) {
	...
	const firstSyncBody = `<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:">
  <d:response>
    <d:href>/calendar/phantom-invite.ics</d:href>
    <d:propstat>
  ...
  <d:sync-token>https://example.com/sync/t1</d:sync-token>
	...
	const nextSyncBody = `<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:">
  <d:sync-token>https://example.com/sync/t2</d:sync-token>
</d:multistatus>`

	const phantomICS = `BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//chroncal//tests//EN
BEGIN:VEVENT
UID:late-invite
...
`
	...
	client := newTestCalDAVClient(t, func(r *http.Request) (*http.Response, error) {
		...
		if !strings.Contains(body, "calendar-multiget") {
			syncBody := firstSyncBody
			if strings.Contains(body, "https://example.com/sync/t1") {
				syncBody = nextSyncBody
			}
			return &http.Response{
				StatusCode: http.StatusMultiStatus,
				Status:     "207 Multi-Status",
		...
		multigetN++
		if !strings.Contains(body, "/calendar/phantom-invite.ics") {
			t.Fatalf("multiget %d omitted the pending href", multigetN)
		}
		if multigetN == 1 {
			return &http.Response{
				StatusCode: http.StatusMultiStatus,
  ...
  <d:response>
    ...
    <d:propstat>
      <d:prop>
        ...
        <cal:calendar-data>` + phantomICS + `</cal:calendar-data>
      </d:prop>
      <d:status>HTTP/1.1 200 OK</d:status>
    </d:propstat>
  </d:response>
	...
	if multigetN != 2 {
		t.Fatalf("multiget calls = %d, want 2", multigetN)
	}
	pending, err := q.ListSyncPendingHrefsByCalendar(ctx, calendarID)
	if err != nil {
		t.Fatalf("ListSyncPendingHrefsByCalendar: %v", err)
	}
	if len(pending) != 0 {
		t.Fatalf("pending hrefs = %d, want 0 after a successful retry", len(pending))
	}
}
...
<d:multistatus xmlns:d="DAV:">
  ...
  <d:sync-token>https://example.com/sync/t1</d:sync-token>
	...
	const laterSyncBody = `<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:">
  <d:sync-token>https://example.com/sync/t2</d:sync-token>
</d:multistatus>`
	...
	client := newTestCalDAVClient(t, func(r *http.Request) (*http.Response, error) {
		...
		if !strings.Contains(body, "calendar-multiget") {
			syncBody := firstSyncBody
			if strings.Contains(body, "https://example.com/sync/") {
				syncBody = laterSyncBody
			}
			return &http.Response{
				StatusCode: http.StatusMultiStatus,
				Status:     "207 Multi-Status",
...
<d:multistatus xmlns:d="DAV:">
  <d:response><d:href>/calendar/phantom-invite.ics</d:href><d:status>HTTP/1.1 404 Not Found</d:status></d:response>
	...
	if len(pending) != 0 {
		t.Fatalf("pending hrefs = %d, want 0 after the miss budget", len(pending))
	}
}
